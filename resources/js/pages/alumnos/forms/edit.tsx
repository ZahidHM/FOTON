...
export function EditForm({ alumno, materias }: Props) {
  console.log(materias);
  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
    defaultValues: {
      nombre: alumno.nombre,
      apellido_pat: alumno.apellido_pat,
      apellido_mat: alumno.apellido_mat,
      edad: String(alumno.edad),
      matricula: alumno.matricula,
      grado: alumno.grado,
      materias: alumno.materias?.map((m: any) => String(m.id)) || [],
    },
  })

  function onSubmit(values: z.infer<typeof formSchema>) {
    router.put(`/alumnos/${alumno.id}`, values, {
      onError: (errors) => {
